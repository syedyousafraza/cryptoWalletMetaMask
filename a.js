...
(async () => {
  ...
  const userDataDir = 'C:UsersYousafAppData/Local/Google/Chrome/User Data/Default';
...
//else finish here}
  ...
  await extensionPage.click('[data-testid="request-signature__sign"]',{delay: 3000});

  //await expect(newtab).toHaveURL('https://goerli-test.zed.run/stable/stable-94224685');

  await newtab.waitForSelector('.header-container > .header > .header-content > .right-part > .balance-part')
  await newtab.click('.header-container > .header > .header-content > .right-part > .balance-part')

  await newtab.click('text=Deposit');
  await newtab.click('[placeholder="Enter Amount..."]');
  ...
  {delay: 10000};
  await extensionPage.reload();
  await extensionPage.waitForSelector('[data-testid="page-container-footer-next"]');
  await extensionPage.click('[data-testid="page-container-footer-next"]');

//Loggin out from the application
